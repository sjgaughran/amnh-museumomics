...
library(LEA)
#read in the vcf
vcf <- read.vcfR("Desktop/Tminimus_SS_minQ20kminDP100_GenoDP3GQ20DP150_bi_lowmiss_noTransit_12klines.vcf")
#make it a genind object for adegenet
data <- vcfR2genind(vcf)
#This program allows you to explore structure in your dataset.
...
#Often, it can be difficult to pinpoint the best k for your data. 
#A good question to ask is ”How many clusters are useful to describe the data?" 
#Clusters are merely tools used to summarise and understand the data, rather than test hypotheses.
#There is no "true k”,but some values of k are better and more efficient summaries of the data than others.
#You can read more about it in the DAPC tutorial here: https://adegenet.r-forge.r-project.org/files/tutorial-dapc.pdf
#so, let's test up to 10 clusters! 
grp<-find.clusters(data,max.n.clust=10)
#Let's reduce the number of PCs to have a clearer picture of the data. Let's go with 50. 
#We generally want to pick the number of clusters with the smallest BIC- in this case, 2. But again, this is an art not a science! 
...
#this algorithm is prone to overfitting, so let's retain ~80 PCs.
...
#Not very interesting, since we only split the data into two groups. But let's see which samples are associated with each population! 
...
#Take a look at our population file on github. It looks like the modern samples are clustering together and the and historical samples are clustering together!
...
assignplot(dapc1, cex.lab = 0.4)
#Again, let's compare to our pop file. Looks like there is a divide between modern and historical, as well as North/South!
...
#Usually we try to choose the "knee"- so, usually the lowest in this CE graph is probably best. But, again! Think about what is biologicall meaningful to your question.
...
#This script is using a genomic control, which helps reduce the effects of population structure.
#The lambda value is used as an inflation factor to re-scale the chi-squared statistics in the computation of p-values. 
...
#And K=4 
p4 = snmf.pvalues(project1, entropy = TRUE, ploidy = 2, K = 4, genomic.control = T, lambda = 2.5) 
pvalues4 = p4$pvalues
par(mfrow = c(2,1)) 
hist(pvalues4, col = "orange") 
plot(-log10(pvalues4), pch = 19, col = "blue", cex = .5)
